...
void display_R(char *path)             //-R参数
{
    ...
    DIR *dir;
    struct dirent *ptr;
    char allname[251][261],name[251][261],a[251],b[251];
    int i,j,k,len,count;
...
void display_dir(int flag_param,char *path)             
{
    ...
    struct dirent *ptr;
    int count=0;
    char filename[256][PATH_MAX+1],fullname[256][PATH_MAX+1],name[251][261];
    char temp[260];
    ...
    for(i = 0;i<count-1;i++)
        ...
        if(flag_param == 9 || flag_param == 13 || flag_param == 15)
        {
            for(i=0;i<count;i++)
            {
                int flag=1;
